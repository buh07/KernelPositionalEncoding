...
\begin{abstract}
We present a unified framework connecting LayerNorm/RMSNorm geometry, positional encodings as group representations, attention as a kernel machine, and Fourier representations of shift-invariant kernels. The core thesis remains that under relative positional encodings (especially RoPE) and token-wise normalization, the positional component of attention is well-approximated by a shift-invariant kernel over relative offsets, with known approximation limits under absolute encodings, ALiBi, and causal boundaries.

This full revision updates the paper's empirical status from ``proposed protocols'' to a mixed completed+pending state anchored to current artifacts. Experiment 1 constraints are now empirically established (including Track B raw invariance across centering variants and heterogeneous centered-diagnostic recovery), while the authoritative Experiment 2 Phase 2B confirmatory gate remains pending because centered posthoc completion is not yet finished. Accordingly, this document explicitly separates confirmed findings, interim descriptive evidence, and unresolved confirmatory hypotheses (H1--H5), and it applies conservative claim guardrails until the blocked Phase 2B gate is resolved.
\end{abstract}
...
This paper synthesizes individually established results---LayerNorm geometry \cite{ba2016layer}, positional encodings as group representations \cite{vaswani2017attention,su2021roformer,lim2024positional}, attention as kernel smoothing \cite{tsai2019transformer}, and Random Fourier Features \cite{rahimi2007random}---into a coherent approximate framework and now reports the current empirical status against that framework. The specific contributions in this revised version are:
\begin{enumerate}
    ...
    \item \textbf{Evidence-locked empirical update}: Experiment 1 results are complete and constrain interpretation; Experiment 2 is partially complete with authoritative Phase 2B currently blocked for confirmatory adjudication due to pending centered posthoc completion.
    \item \textbf{Conservative claim adjudication policy}: confirmed vs interim vs pending claims are explicitly separated; no confirmatory H1/H2/H3 language is used while Phase 2B remains blocked.
    \item \textbf{Connections to kernel-attention methods} (Performers \cite{choromanski2021performers}, linear attention \cite{katharopoulos2020linear}), clarifying how this interpretability-focused framework complements efficiency-focused kernel approximations.
\end{enumerate}

Snapshot and evidence lock for this revision:
\begin{itemize}
    \item \textbf{Snapshot timestamp:} 2026-03-09 16:22:45 (artifact time basis for Experiment 2 gate/decision status).
    \item \textbf{Authoritative sources:} \texttt{experiment1/experiment1results.md}, \texttt{results/reports/track\_b\_granularity\_norm\_v1/summary.md}, \texttt{results/experiment2/phase2b/exp2\_phase2b\_fastq\_v8\_20260307\_170106/*}, and \texttt{experiment2/long\_offset\_lock.json}.
    \item \textbf{Status split:} Experiment 1 = completed and confirmatory-relevant; Experiment 2 = execution advanced but confirmatory adjudication pending; Experiments 3--7 = still prospective in this manuscript.
\end{itemize}
...
\textbf{Status note for this revision:} this section no longer uses a single ``all proposed'' framing. It now contains (i) completed evidence (Experiment 1), (ii) current in-progress evidence (Experiment 2 authoritative run), and (iii) prospective protocols (Experiments 3--7).

The preceding sections establish both when the framework should work (Sections~\ref{sec:ln}--\ref{sec:synthesis}: early layers, relative PEs) and why it is expected to degrade in deeper entangled regimes (Section~\ref{sec:residual-mlp}). This revised validation section is organized around:
\begin{enumerate}
    \item \textbf{Completed evidence}: finalized Experiment 1 outputs that now constrain methodology and interpretation for downstream claims.
    \item \textbf{Current in-progress evidence}: Experiment 2 Phase 2B run state and why confirmatory adjudication is currently blocked.
    \item \textbf{Future protocols}: Experiments 3--7 remain specified as prospective tests.
\end{enumerate}

\paragraph{Conservative adjudication policy.}
For Experiment 2 confirmatory hypotheses (H1/H2/H3), this manuscript applies a strict gate policy: no confirmatory pass/fail claims are allowed while the authoritative run reports \texttt{status=blocked\_centered\_pending}. H3 remains a co-occurrence/specificity criterion, not mediation identification.

\begin{table}[ht]
\centering
\caption{Table A: Claim status board (snapshot-locked)}
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.10\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}X}
\toprule
\textbf{ID} & \textbf{Claim} & \textbf{Status} & \textbf{Primary evidence (locked source)} \\
\midrule
C1 & Track B raw invariance across centering variants (Exp1) & \textbf{Confirmed} & \texttt{track\_b\_granularity\_norm\_v1/summary.md}: raw invariance diffs are zero across tested variants. \\
C2 & Legacy vs canonical per-position centered equivalence (Exp1) & \textbf{Confirmed} & Same summary: centered equivalence mean abs diff 0, max abs diff $10^{-5}$. \\
C3 & Synthetic centered$\approx$raw (Exp1) & \textbf{Confirmed} & Same summary: synthetic centered==raw diff 0. \\
C4 & Branch B heterogeneous recovery is correct (Exp1) & \textbf{Confirmed} & Same summary + Exp1 report: monotonic recovery under coarser centering with norm/family heterogeneity. \\
C5 & Long-range lock calibrated and applied (Exp2) & \textbf{Confirmed} & \texttt{experiment2/long\_offset\_lock.json}: status ok, selected [8,12], fallback [8]. \\
H1 & High-frequency ablation: short $>$ long degradation & \textbf{Pending} & Phase 2B gate blocked pending centered completion. \\
H2 & Low-frequency ablation: long $>$ short degradation & \textbf{Pending} & Phase 2B gate blocked pending centered completion. \\
H3 & Targeted-vs-random specificity & \textbf{Pending} & Confirmatory H3 block not evaluable in blocked gate state. \\
H4 & Norm-family interaction claim class & \textbf{Not Evaluable Yet} & Exploratory by protocol in current pipeline. \\
H5 & Tier-1 transfer claim class & \textbf{Not Evaluable Yet} & Exploratory and scope-limited by claim guard metadata. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Table B: Phase 2B execution state (\texttt{exp2\_phase2b\_fastq\_v8\_20260307\_170106})}
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.43\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}X}
\toprule
\textbf{Field} & \textbf{Value} & \textbf{Source} \\
\midrule
Total planned cells & 1932 & \texttt{manifest\_summary.json} (synthetic 924, span\_bridge 756, tier1\_ppl 252). \\
Pruned rows (non-baseline floor-limited) & 640 & \texttt{manifest\_prune\_summary.json}. \\
Kept rows after prune & 1292 & \texttt{manifest\_prune\_summary.json} (252 baseline + 1040 non-baseline). \\
Executed rows (\texttt{run\_config} rows) & 1231 & \texttt{execution\_coverage.json}. \\
Missing rows (vs full manifest) & 701 & \texttt{execution\_coverage.json}. \\
Centered pending cells & 1151 & \texttt{decision\_summary.json}, \texttt{gate\_evaluation.json}. \\
Gate status & blocked\_centered\_pending & \texttt{gate\_evaluation.json}. \\
Phase2B confirmatory pass observed & unknown & \texttt{promotion\_guard.json}. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Table C: Long-offset lock summary}
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.36\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}X}
\toprule
\textbf{Lock parameter} & \textbf{Value} & \textbf{Interpretation} \\
\midrule
Lock status & ok & Valid lock artifact present and active. \\
Source sweep run & exp2\_feas\_amended\_fast\_20260306\_234245 & Baseline-only feasibility sweep source. \\
Selection policy & per\_task\_model\_majority & Per-task model-support rule. \\
Threshold & 0.15 & Floor threshold used in lock recommendation. \\
Required pass/support fraction & $2/3$ ($0.6667$) & Minimum model support per long task. \\
Selected long offsets & [8, 12] & Active long-range spans for delayed/retrieval long tasks. \\
Fallback spans & [8] & Floor fallback span set. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Table D: Baseline floor-limited diagnostics by task/model (interim, descriptive)}
\footnotesize
...
synthetic & long range retrieval & tinyllama-1.1b & 5 & 0 & 0.000 & 0.1726 \\
...
\begin{table}[ht]
\centering
\caption{Table E: H1--H5 adjudication state and exact resolution conditions}
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.07\textwidth}>{\raggedright\arraybackslash}p{0.33\textwidth}>{\raggedright\arraybackslash}p{0.16\textwidth}X}
\toprule
\textbf{Hyp.} & \textbf{Current evidence} & \textbf{Status} & \textbf{Required to resolve} \\
\midrule
H1 & Interim synthetic contrasts exist in partial outputs; final gate not produced. & \textbf{Pending} & Complete centered posthoc and run final reanalysis to produce non-blocked Phase 2B gate labels. \\
H2 & Interim synthetic contrasts currently trend opposite expected direction in partial outputs; still non-final. & \textbf{Pending} & Same as H1; only final non-blocked gate output is adjudicative. \\
H3 & Random-control structure is implemented; confirmatory specificity gate block not available yet. & \textbf{Pending} & Same as H1/H2 plus final H3 block population in gate payload. \\
H4 & Norm interaction signal exists as exploratory decomposition path. & \textbf{Not Evaluable Yet} & Remains exploratory unless protocol class is changed. \\
H5 & Tier-1 stratified outputs exist; claim guard limits inference scope. & \textbf{Not Evaluable Yet} & Remains exploratory; no confirmatory promotion under current policy. \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Meaning of success/failure in this mixed-status setting.}
For completed Experiment 1 analyses, success/failure statements are empirical and fixed by existing artifacts. For Experiment 2, success/failure for confirmatory hypotheses remains unresolved until the blocked gate state clears. For Experiments 3--7, success/failure criteria remain prospective as defined in their protocol subsections below.

\subsection{Experiment 1: Estimating the Positional Kernel From Queries and Keys}
\label{sec:exp-pos-kernel}

\paragraph{Status in this revision: \textbf{Completed}.}
Experiment 1 is complete and evidence-locked in this revision. The authoritative summaries are \texttt{experiment1/experiment1results.md} and \texttt{results/reports/track\_b\_granularity\_norm\_v1/summary.md}. In particular, Track B raw invariance across centering variants is confirmed, synthetic centered-vs-raw parity is confirmed, and heterogeneous centered recovery under coarser centering supports the Experiment 1 Branch B resolution used to constrain Experiment 2 diagnostics.
...
\paragraph{Status in this revision: \textbf{In Progress, Confirmatory-Adjudication Blocked}.}
The authoritative Experiment 2 Phase 2B run in this revision is \texttt{exp2\_phase2b\_fastq\_v8\_20260307\_170106}. Execution has advanced, but confirmatory adjudication remains blocked while centered posthoc backlog is unresolved (\texttt{status=blocked\_centered\_pending}). Therefore, all H1/H2/H3 statements in this document are interim and non-confirmatory until posthoc-centered completion and final reanalysis clear the gate.
...
\paragraph{Status in this revision: \textbf{Prospective/Future Work}.}
Experiment 3 remains a planned protocol in this revision and has not been executed in the authoritative snapshot.
...
\paragraph{Status in this revision: \textbf{Prospective/Future Work}.}
Experiment 4 remains a planned protocol in this revision and has not been executed in the authoritative snapshot.
...
\paragraph{Status in this revision: \textbf{Prospective/Future Work}.}
Experiment 5 remains a planned protocol in this revision and has not been executed in the authoritative snapshot.
...
\paragraph{Status in this revision: \textbf{Prospective/Future Work}.}
Experiment 6 remains a planned protocol in this revision and has not been executed in the authoritative snapshot.
...
\paragraph{Status in this revision: \textbf{Prospective/Future Work}.}
Experiment 7 remains a planned protocol in this revision and has not been executed in the authoritative snapshot.
...
\begin{itemize}
    ...
    \item \textbf{Empirical status is mixed (not fully complete):} Experiment 1 is complete and constrains methodology; Experiment 2 is in progress with confirmatory adjudication blocked while centered posthoc is pending; Experiments 3--7 remain unrun protocols in this revision.
    \item \textbf{Claim-scope guardrails for current Experiment 2 state:} while \texttt{status=blocked\_centered\_pending}, no confirmatory H1/H2/H3 pass/fail claims are valid. H3 should be interpreted as co-occurrence/specificity evidence, not mediation identification.
    \item \textbf{Training variance not measured in current Experiment 2 evidence:} current inference scope is task-sampling variability under fixed checkpoints; training-run-to-training-run generalization is out of scope for the present snapshot.
