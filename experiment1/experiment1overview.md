...
**Post-run note (interpretation update, 2026-02-26):** Experiment 1 supports a stronger version of this caution: the observed natural-text centered collapse is consistent with the centering procedure removing a large structured component.

**Post-run note (centering granularity sweep resolution, 2026-03-03):** The centering granularity sweep is now complete. Testing 6 centering strategies (per-position, bucketed b8/b16/b32/b64, shared-mean) across all 6 models confirms that per-position centering was the dominant confound. Key findings:
- **RMSNorm models (LLaMA, TinyLlama):** shared-mean centered R² = raw R² exactly (max |Δ| = 5.55e-16). This is an algebraic identity: `_normalize_logits_for_norm_batch` double-centers RMSNorm logits, which cancels the rank-1 terms from shared-mean subtraction.
- **LayerNorm models (OLMo):** shared-mean centered R² = 108.6% of raw, indicating content-dependent offsets depress raw R². GPT-2 (learned PE + LayerNorm) shows 100.8–107.8%.
- **Recovery is monotonic** for 80.6% of RoPE head/layer combos across the granularity sweep.
- **Branch selection:** Branch B (heterogeneous, norm-family-specific recovery) is confirmed. This resolves the "shared-mean or canonical-frame variants" question raised in the earlier note.
