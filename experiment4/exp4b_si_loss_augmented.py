...
from experiment4.pipeline import TrainConfig, run_4b_model
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Experiment 4B: SI-loss-augmented FT")
    p.add_argument("--model", choices=TARGET_MODELS, required=True)
    p.add_argument("--device", default="cuda:0")
    p.add_argument("--seeds", default="0,1,2")
    p.add_argument("--lambdas", default="0.01,0.1,1.0")
    p.add_argument("--output-root", default=str(RESULTS_ROOT / "exp4b_si_loss_augmented"))
    ...
    p.add_argument("--aux-interval", type=int, default=10)
...
def main() -> None:
    ...
    manifest = {
        ...
        "lora": hp.__dict__,
        "artifacts": {
            "si_loss_sweep": str(out_root / str(args.model) / "si_loss_sweep.json"),
            "si_routing_results": str(out_root / str(args.model) / "si_routing_results.json"),
            "post_ft_si_audit": str(out_root / str(args.model) / "post_ft_si_audit.json"),
            "runs_table": str(out_root / str(args.model) / "si_loss_runs.parquet"),
        },
        "summary": summary,
    }
    write_json(out_root / str(args.model) / "run_manifest.json", manifest)
    print(f"[4B] completed: {out_root / str(args.model) / 'run_manifest.json'}")
