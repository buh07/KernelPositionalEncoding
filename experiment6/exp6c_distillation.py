#!/usr/bin/env python3
"""CLI entrypoint for Experiment 6C: Two-Phase SI Distillation."""
...
from experiment6.pipeline import run_6c
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Experiment 6C: Two-Phase SI Distillation")
    p.add_argument("--model", choices=TARGET_MODELS, required=True)
    p.add_argument("--device", default="cuda:0")
    p.add_argument("--seeds", default="0,1,2")
    p.add_argument("--output-root", default=str(RESULTS_ROOT / "exp6c_distillation"))
...
def main() -> None:
    ...
    print(f"[6C] completed: {out_root / str(args.model) / 'run_manifest.json'}")
