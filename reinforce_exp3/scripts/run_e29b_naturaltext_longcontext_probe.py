#!/usr/bin/env python3
"""E29B — Natural-text long-context SI utility probe.

Purpose
-------
Assess whether SI-kernel subtraction preferentially disrupts retrievable mappings
embedded in longer, semi-natural wiki/code contexts versus matched controls that
preserve length/style but break mapping consistency.
...
Primary endpoint
----------------
Pooled preferential gap (true SI subtraction):
    gap_true = (intact_lp_probe - true_lp_probe) - (intact_lp_ctrl - true_lp_ctrl)
Support if bootstrap CI lower bound > 0.

Secondary endpoint
------------------
Permutation-specificity on the preferential gap:
    gap_true_minus_perm = gap_true - gap_perm
"""
...
EXPERIMENT_ID = "E29B"
OUT_ROOT = RESULTS_ROOT / "E29b_naturaltext_longcontext_probe"
...
FAMILIES: list[FamilySpec] = [
    FamilySpec("wiki_passkey_256", domain="wiki", context_len=256, n_demos=4, offset=3),
    FamilySpec("wiki_passkey_512", domain="wiki", context_len=512, n_demos=6, offset=4),
    FamilySpec("code_passkey_512", domain="code", context_len=512, n_demos=6, offset=4),
]
...
def _load_kernels(model_name: str) -> dict[tuple[int, int], np.ndarray]:
    for template in _KERNEL_CANDIDATES:
        ...
        if out:
            return out
    raise FileNotFoundError(f"[E29B] No estimated_kernels.json for {model_name}")
...
def _build_prompt_pair(
    *,
    ...
    # Keep one slot for query token; target is predicted next-token.
    ...
    # Demo placements spread across the full context.
    start = 16
    end = max(start + spec.n_demos + spec.offset + 8, len(ctx) - 24)
    ...
    for p in pos:
        key_pos = int(max(4, min(len(ctx) - 6, int(p))))
        val_pos = int(max(key_pos + 1, min(len(ctx) - 3, key_pos + int(spec.offset))))

        # Probe: same key/value mapping repeated.
        probe[key_pos] = key
        probe[val_pos] = val

        # Control: per-demo independent mappings.
    ...
    *,
    ...
    if spec.domain == "wiki":
        return _load_wiki_sequences(
            model_name,
            tokenizer,
            seq_len=int(spec.context_len - 1),
            max_sequences=max_sequences,
            seed=seed,
        )
    if spec.domain == "code":
        return _load_code_sequences(
            model_name,
            tokenizer,
            seq_len=int(spec.context_len - 1),
            max_sequences=max_sequences,
...
def _build_family_prompts(
    *,
    ...
    if len(probe) < max(8, n_prompts // 2):
        raise RuntimeError(
            f"[E29B] insufficient prompts for {model_name}/{spec.name}: {len(probe)} < {max(8, n_prompts // 2)}"
        )
...
def run_model(
    *,
    ...
    if not high_heads:
        raise RuntimeError(f"[E29B] no high_si heads available for {model_name}")
    ...
    for fidx, spec in enumerate(FAMILIES):
        ...
        fam_ci = _bootstrap_mean_ci(gap_true, n_boot=3000, seed=seed + 4000 + fidx)
        fam_ci_delta = _bootstrap_mean_ci(gap_true - gap_perm, n_boot=3000, seed=seed + 5000 + fidx)
            ...
            trial_rows.append(
                {
                    ...
                    "loss_true_probe_lp": float(loss_true_probe[i]),
                    "loss_true_ctrl_lp": float(loss_true_ctrl[i]),
                    "loss_perm_probe_lp": float(loss_perm_probe[i]),
                    "loss_perm_ctrl_lp": float(loss_perm_ctrl[i]),
                }
            )

        print(
            f"[E29B][{model_name}] {spec.name}: "
            f"gap_true={fam_row['mean_gap_true_lp']:.6f} "
            f"gap_perm={fam_row['mean_gap_perm_lp']:.6f} "
    ...
    pooled_true_minus_perm = trials_df["gap_true_minus_perm_lp"].to_numpy(dtype=np.float64)

    pooled_ci = _bootstrap_mean_ci(pooled_true, n_boot=5000, seed=seed + 9001)
    pooled_ci_delta = _bootstrap_mean_ci(pooled_true_minus_perm, n_boot=5000, seed=seed + 9002)

    model_pass = bool(float(pooled_ci["ci_lo"]) > 0.0)
    rec = {
        ...
        "mean_pooled_gap_true_lp": float(pooled_ci["mean"]),
        "pooled_gap_true_ci": [float(pooled_ci["ci_lo"]), float(pooled_ci["ci_hi"])],
        "p_one_gt_zero": float(pooled_ci["p_one_gt_zero"]),
        "mean_pooled_gap_true_minus_perm_lp": float(pooled_ci_delta["mean"]),
        "pooled_gap_true_minus_perm_ci": [float(pooled_ci_delta["ci_lo"]), float(pooled_ci_delta["ci_hi"])],
        "model_directional_pass": model_pass,
    ...
    print(
        f"[E29B][{model_name}] pooled_true={rec['mean_pooled_gap_true_lp']:.6f} "
        f"ci=[{pooled_ci['ci_lo']:.6f},{pooled_ci['ci_hi']:.6f}] pass={model_pass}",
        flush=True,
    )
    return rec
...
def _finalize(models: list[str], out_root: Path) -> dict[str, Any]:
    ...
    for m in models:
        ...
        if not p.exists():
            raise RuntimeError(f"[E29B] hard_fail_reason: missing summary for {m}: {p}")
    ...
    if n_pass == len(rows):
        interp = "naturaltext_longcontext_preferential_gap_supported"
        status = "supported"
    elif n_pass >= 2:
        interp = "naturaltext_longcontext_preferential_gap_supported_with_caveat"
        status = "supported_with_caveat"
    elif n_pass >= 1:
        interp = "naturaltext_longcontext_preferential_gap_mixed"
        status = "mixed"
    else:
        interp = "naturaltext_longcontext_preferential_gap_not_supported"
    ...
    prereg = {
        "experiment_id": EXPERIMENT_ID,
        "question": "Does SI subtraction preferentially disrupt long-context retrievable mappings over matched controls?",
        "primary_hypothesis": "Pooled true preferential gap > 0 in LP units.",
        "primary_endpoints": [
            "mean_pooled_gap_true_lp",
            "pooled_gap_true_ci",
            "mean_pooled_gap_true_minus_perm_lp",
        ],
    }
    ...
    summary = {
        ...
        "limitations": [
            "Prompts are semi-naturalistic constructions, not benchmark-grade downstream tasks.",
            "Kernel subtraction is evaluation-time disruption and not a training counterfactual.",
        ],
    }
    ...
    claim_impact = {
        ...
        "notes": [
            "Provides bounded natural-text long-context utility evidence for SI interventions.",
        ],
    }
        ...
        "tables": [
            {
                ...
                "description": "Family-level preferential-gap summaries for true vs permuted interventions.",
                "columns": [],
            },
            {
                "path": "<model>/prompt_level_gaps.parquet",
                "description": "Per-prompt preferential-gap deltas used for pooled inference.",
                "columns": [],
            },
        ],
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="E29B: natural-text long-context SI utility probe", allow_abbrev=False)
    ...
    p.add_argument("--n-prompts", type=int, default=72)
    ...
    if args.finalize_only and args.no_finalize:
        raise RuntimeError("[E29B] hard_fail_reason: --finalize-only and --no-finalize are mutually exclusive")
    ...
    if args.smoke:
        n_prompts = min(n_prompts, 20)
        batch_size = min(batch_size, 6)
        n_control_trials = min(n_control_trials, 1)

    if args.finalize_only:
        cross = _finalize(models, out_root)
        print(f"[E29B] Finalized. interpretation={cross['interpretation']}", flush=True)
        return

    for model_name in models:
        device = device_map.get(model_name, "cuda:0")
        print(f"[E29B] Running {model_name} on {device}", flush=True)
        run_model(
            model_name=model_name,
            device=device,
            out_root=out_root,
    ...
    if args.no_finalize:
        print("[E29B] Shard complete (no finalize).", flush=True)
        return

    cross = _finalize(models, out_root)
    print(f"[E29B] Done. interpretation={cross['interpretation']}", flush=True)
