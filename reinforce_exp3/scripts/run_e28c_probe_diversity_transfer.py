#!/usr/bin/env python3
"""E28c — Probe-Diversity Transfer Battery for Result III.

Purpose
-------
Address the critique that E12's 9-token ICL probe is structurally aligned with
SI by design. E28c tests whether preferential disruption under SI-kernel
...
- Probe families (7): baseline E28 families plus three lower-regularity
  20–50-token transfer families
...
Model-level primary pass requires >= 75% family passes.
...
OUT_ROOT = RESULTS_ROOT / "E28c_probe_diversity_transfer"
SEED_BASE = 20260503
...
FAMILIES: list[FamilySpec] = [
    ...
    FamilySpec("jittered_mid_offset", n_demos=6, inpair_gap=(1, 3), between_gap=(1, 2)),  # ~28-38
    FamilySpec("sparse_long_offset", n_demos=6, inpair_gap=(2, 4), between_gap=(2, 3)),   # ~37-49
    FamilySpec("broad_jitter_offset", n_demos=7, inpair_gap=(0, 3), between_gap=(1, 3)),  # ~22-50
]
...
def _load_kernels(model_name: str) -> tuple[dict[tuple[int, int], np.ndarray], str]:
    ...
    raise FileNotFoundError(
        f"[E28c] No estimated_kernels.json found for {model_name}; checked: {candidates}"
    )
...
def run_model(
    *,
    ...
    print(f"[E28c] loading {model_name} on {device}", flush=True)
    ...
    if not high_heads:
        raise RuntimeError(f"[E28c] no high_si heads available for {model_name}")
    ...
    for idx, spec in enumerate(FAMILIES):
        ...
        print(
            f"[E28c] {model_name} {spec.name}: gap_true_lp={fam['gap_true_lp']:.4f} "
    ...
    # model-level primary pass: >=75% LP-family passes (rounded up).
    n_required = int(np.ceil(0.75 * max(n_family, 1)))
    model_pass = bool(n_pass_lp >= n_required)
    ...
    summary = {
        ...
        "n_required_family_pass_lp": int(n_required),
...
def _load_model_summary(model_name: str, out_root: Path) -> dict[str, Any]:
    ...
    if not p.exists():
        raise RuntimeError(f"[E28c] hard_fail_reason: missing shard artifact {p}")
    return read_json(p)
...
def _finalize(models: list[str], out_root: Path) -> dict[str, Any]:
    ...
    if n_pass == len(rows):
        ...
        note = "All models pass the multi-family probe-scope gate (>=75% LP-family passes)."
    ...
    write_json(out_root / "cross_model_probe_diversity_summary.json", cross)
    ...
    prereg = {
        ...
        "primary_endpoints": [
            ...
            "model_primary_pass_geq_75pct_families",
        ],
        "secondary_endpoints": [
            "family_gap_true_acc",
...
def main() -> None:
    p = argparse.ArgumentParser(description="E28c: probe-diversity transfer battery for Result III", allow_abbrev=False)
    ...
    if args.finalize_only and args.no_finalize:
        raise RuntimeError("[E28c] hard_fail_reason: --finalize-only and --no-finalize are mutually exclusive")
    ...
    if args.finalize_only:
        ...
        print(f"[E28c] Finalized from shards. interpretation={cross['interpretation']}", flush=True)
        return
    ...
    if args.no_finalize:
        print("[E28c] Shard run complete (no finalize).", flush=True)
        return

    cross = _finalize(models, out_root)
    print(f"[E28c] Done. interpretation={cross['interpretation']}", flush=True)
