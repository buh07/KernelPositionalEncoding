#!/usr/bin/env python3
"""E31D: Llama-only naturalistic SI vs importance-matched non-SI control.

Fast add-on extending E29B-style natural-text passkey prompts with a direct arm:
    SI-targeted subtraction vs importance-matched non-SI subtraction.
"""
...
from reinforce_exp3.scripts._shared import load_model_for_exp  # noqa: E402
from experiment3.theory8_position_ablation import subtract_positional_kernels  # noqa: E402

EXPERIMENT_ID = "E31D"
DEFAULT_OUT = RESULTS_ROOT / "E31d_llama_naturalistic_importance_control"
MODEL = "llama-3.1-8b"


def _load_importance_heads(model_name: str) -> list[tuple[int, int]]:
    sel = (
        ROOT
        / "results"
        / "reinforce_exp3"
    ...
    # Fallback: derive from cached importance proxy + non-SI mask.
    proxy_path = (
        ROOT
        / "results"
        / "reinforce_exp3"
    ...
    r2_path = ROOT / "results" / "experiment3" / "theory1_si_circuits" / model_name / "head_r2_summary.parquet"
    ...
    if not (proxy_path.exists() and r2_path.exists() and groups_path.exists()):
        raise FileNotFoundError("[E31D] Missing E28B head-set artifacts and fallback inputs")
    ...
    # Match high-SI cardinality.
    n_high = len(high)
    out = [(int(r.layer), int(r.head)) for r in non.head(n_high).itertuples(index=False)]
    if not out:
        raise RuntimeError("[E31D] Could not derive fallback importance-matched non-SI heads")
    return out


def _one_sided_signflip_p(diff: np.ndarray, n_perm: int = 20000, seed: int = 20260504) -> float:
...
def run(
    *,
    ...
    # High-SI heads from E29B path.
    groups_path = ROOT / "results" / "experiment3" / "theory1_si_circuits" / MODEL / "head_groups.json"
    groups = json.loads(groups_path.read_text(encoding="utf-8"))
    high_heads = [(int(d["layer"]), int(d["head"])) for d in groups.get("high_si", []) if (int(d["layer"]), int(d["head"])) in kernels]
    if not high_heads:
        raise RuntimeError("[E31D] no usable high-SI heads for Llama")

    imp_heads = [h for h in _load_importance_heads(MODEL) if h in kernels]
    if not imp_heads:
        raise RuntimeError("[E31D] no usable importance-matched non-SI heads")
    if len(imp_heads) != len(high_heads):
        # Strict cardinality match to keep the arm clean.
    ...
    for fidx, spec in enumerate(FAMILIES):
        ...
        with subtract_positional_kernels(model, kernels, imp_heads, max_len):
            imp_probe = _eval_target_metrics(
                model=model,
                prompts=probe_prompts,
                device=device,
                pad_token_id=pad_id,
        ...
        print(
            f"[E31D][{spec.name}] mean_gap_si={np.mean(gap_si):.6f} "
            f"mean_gap_imp={np.mean(gap_imp):.6f} mean_delta={np.mean(gap_si_minus_imp):.6f}",
            flush=True,
        )

    trials_df = pd.DataFrame(trial_rows)
    trials_df.to_parquet(model_dir / "llama_naturalistic_arm_trials.parquet", index=False)
    ...
    write_json(out_root / "llama_naturalistic_importance_vs_si.json", result)

    patch = f"""# Result III Add-on Patch (E31D)

On semi-naturalistic passkey-style prompts (Llama-only), SI-targeted subtraction yields larger preferential degradation than
importance-matched non-SI subtraction: Δ(SI−IMP)={result['mean_delta_si_minus_imp']:.3f} LP
(CI [{result['delta_si_minus_imp_ci'][0]:.3f}, {result['delta_si_minus_imp_ci'][1]:.3f}], one-sided p={result['p_one_sided_si_gt_imp']:.4g}).

This is a bounded single-model extension and should be interpreted as directional strengthening of Result III scope.
"""
    (out_root / "resultIII_addon_patch.md").write_text(patch, encoding="utf-8")
    ...
    summary = {
        ...
        "override_used": False,
        "verdict": {
            "interpretation": "llama_naturalistic_si_vs_importance_supported"
            if bool(result["directional_support_si_gt_importance"])
            else "llama_naturalistic_si_vs_importance_mixed",
    ...
    prereg = {
        "experiment_id": EXPERIMENT_ID,
        "question": "On semi-naturalistic prompts, is SI-targeted ablation more disruptive than importance-matched non-SI ablation?",
    ...
    claim_impact = {
        ...
        "impact": "adds fast naturalistic-style SI-vs-importance comparison for Result III",
    }
    data_dict = {
        "llama_naturalistic_importance_vs_si.json": "Top-line Llama add-on SI vs importance-matched summary.",
        "llama_naturalistic_arm_trials.parquet": "Prompt-level trial table across SI/permuted/importance arms.",
        "resultIII_addon_patch.md": "Paper-ready wording block for Result III add-on.",
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="E31D Llama naturalistic SI vs importance-matched control")
