...
CUDA_VISIBLE_DEVICES=4 .venv/bin/python -u reinforce_exp/exp_new_r12_ordering_control.py \
  --model llama-3.1-8b --device cuda:0 --output-root "$RUN_ROOT/exp_new_r12_ordering_control_shards/llama_b" \
...
for SEED in 4 12 20; do
  CUDA_VISIBLE_DEVICES=4 .venv/bin/python -u reinforce_exp/exp_r2b_olmo_boundary_power.py run-seed \
